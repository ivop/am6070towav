...
static const char *load_bank(int bank, const char *filename) {
    ...
    static char s[64];
    snprintf(s, 64, "Size: %ld, CRC32: %08x, %s", size, crc, name);
    return s;
}
...
MainWindow::MainWindow(const char *l) : Fl_Double_Window(800, 500, l) {
    int cury = 16;
    ...
    for (int y=0; y<MAX_BANKS; y++) {
        ...
        auto detected = new Fl_Box(curx, cury + y*24, 400, 24);
